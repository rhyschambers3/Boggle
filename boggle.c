...
boggle_t * bg_init(int seed, char * dict_fn){
  ...
  char addedWord [1024];
  while(fscanf(stream, "%s", addedWord) != EOF)
  {
    
    //make all words upper case
    for (char* ptr = addedWord; (*ptr = toupper(*ptr)) != 0; ptr++)
    {
      if (strlen(addedWord) >= 3 && strlen(addedWord) <= 8)
      {
        hm_add(dictionary, addedWord);
      }
    }
...
llist_t * bg_all_words(boggle_t *bg){
  ...
  llist_t * words = ll_init();
   for (int i = 0; i < hm->num_buckets; i++)
   {
    ...
    {
      ll_node_t * cur_n = hm->buckets[i]->head;
      while(cur_n != NULL)
      {
        ll_push(words, cur_n->val);
        cur_n = cur_n -> next;
      }
    }
   }
  ...
  return words; //clearly, this won't be null when your done.
}
...
void _bg_all_words(boggle_t *bg, hashmap_t * hm, int r, int c,
  ...
  //check if columns or rows are greater than the board
  if((r >= 5 || r < 0) || (c < 0 ||c >= 5))
  {
    return;
  }
  ...
  //make current word the last word
  strcpy(cur_word, last_word);
  //go through current word to check if we are at the null terminator
  cur_word[strlen(cur_word)] = bg->board[r][c];
 
  
  //if the next letter is a q, add a u to the end 
  ...
  //check that it is in the dictionary but not in the hashmap already
